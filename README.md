...
- 伪 ChatGPT 桌面端界面
- 对话框输入推进剧情
- 主线使用预设 AI 回复；`LW-____` 会话可在本地通过 DeepSeek 代理模拟林晚人格聊天
- 隐藏线索与密码解谜
- 林晚案主线流程
- 主线任意结局后解锁左侧案件库
...
主线剧情仍然是预设回复。只有左侧 `LW-____` 历史会话中的普通聊天会尝试调用本地 DeepSeek 代理，用来模拟林晚人格。线上 GitHub Pages 版本不会运行代理，也不会接触 API Key。
...
本地代理会为左侧 `LW-____` 会话保留最近几轮对话上下文。玩家输入身世、`23:17`、`LW-2317`、信任或怀疑等指定线索时，会优先触发预设剧情；其他普通聊天内容会尝试调用本地 DeepSeek 代理，让林晚以人格模拟的方式回应。
...
如果不配置 `.env`，或者使用 `python -m http.server`，游戏仍可运行；`LW-____` 的普通聊天会显示固定拒绝回复和系统提示，不会调用真实 API。
